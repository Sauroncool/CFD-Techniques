...
function LW(u, Δt, Δx)
    v = similar(u)
    v[2:end-1] .= v[2:end-1] .= u[2:end-1] .- (((Δt / Δx) .* ((u[2:end-1] .+ u[1:end-2])./2)) ./ 2) .* (u[3:end] .- u[1:end-2]) .+ ((((Δt / Δx) .* ((u[2:end-1] .+ u[1:end-2])./2)).^2) / 2) .* (u[3:end] .- 2 .* u[2:end-1] .+ u[1:end-2])
...
png("3 Single Step Lax Wendroff")