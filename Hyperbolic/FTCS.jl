...
function FTCS(u, c)
    v = copy(u)
    v[1]=u[1]-(c/2)*(u[2]-u[end])
    v[2:end-1] .= u[2:end-1] .- (c/2) .* (u[3:end] .- u[1:end-2])
    v[end]=u[end]-(c/2)*(u[1]-u[end-1])
    return v
end
...
plot(xlabel="x", ylabel="Amplitude",title="FTCS",legend=:outertopleft, grid=true)
