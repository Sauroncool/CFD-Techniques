...
function predictor(u, Δt, Δx) # FTFS Method
    v = copy(u)
    v[2:end-1] .= u[2:end-1] .- ((Δt / Δx) .* ((u[2:end-1] .+ u[1:end-2])./2)) .* (u[3:end] .- u[2:end-1])
    return v
end

function corrector(u, u_prev, Δt, Δx)    # BTBS Method
    v = copy(u)
    v[2:end-1] .= ((u_prev[2:end-1] .+ u[2:end-1]) ./ 2) .- (((Δt / Δx) .* ((u_prev[2:end-1] .+ u_prev[1:end-2])./2)) ./ 2) .* (u[2:end-1] .- u[1:end-2])
    return v
end
